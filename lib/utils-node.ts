...
import { getMySQLArticles, getMySQLArticleBySlug } from "./mysql-blog";
...
export async function getAllArticles(includeHidden = false): Promise<BlogArticle[]> {
  ...
  // 1.5. Obtener de MySQL
  let mysqlArticles: BlogArticle[] = [];
  try {
    const fromMySQL = await getMySQLArticles();
    if (fromMySQL && fromMySQL.length > 0) {
      mysqlArticles = fromMySQL;
      console.log(`Cargados ${fromMySQL.length} artículos desde MySQL`);
    }
  } catch (err) {
    console.error('Error cargando desde mysql-blog', err);
  }
  ...
  // Helper function to normalize categories for legacy data
  const normalizeCategory = (art: BlogArticle) => {
    const ALLOWED = [
    ...
    if (ALLOWED.includes(art.category)) return art.category;

    // Classification mapping (same logic as migration)
    const CLASSIFICATION: Record<string, string[]> = {
      ...
      'activaqr-gastronomia': ['restaurante', 'comid', 'gastronom', 'menu', 'carta', 'qr'],
      'activaqr-networking': ['networking', 'contactos', 'tarjeta', 'vcard']
    };

    const text = (art.title + ' ' + (art.content || '')).toLowerCase();
    let best = 'marketing-para-pymes'; // Default
    let max = -1;

    for (const [cat, kws] of Object.entries(CLASSIFICATION)) {
      let matches = 0;
      kws.forEach(kw => { if (text.includes(kw)) matches++; });
      if (matches > max) { max = matches; best = cat; }
    }
    return best;
  };

  // Combinar y deduplicar por slug (MySQL tiene prioridad, luego Supabase, luego local)
  const mergedArticles = mysqlArticles.map(a => ({ ...a, category: normalizeCategory(a) }));
  const mysqlSlugs = new Set(mysqlArticles.map(a => a.slug));

  for (const supArt of supabaseArticles) {
    if (!mysqlSlugs.has(supArt.slug)) {
      mergedArticles.push({ ...supArt, category: normalizeCategory(supArt) });
    }
  }

  const existingSlugs = new Set(mergedArticles.map(a => a.slug));

  for (const localArticle of allArticles) {
    if (!existingSlugs.has(localArticle.slug)) {
...
export async function getArticle(category: string, slug: string): Promise<BlogArticle | null> {
  ...
  try {
    ...
    // 0.5 Intentar buscar en MySQL
    console.log(`Buscando artículo en MySQL con slug: ${slug}`);
    try {
      const decodedSlugForMySQL = decodeURIComponent(slug);
      const mysqlArticle = await getMySQLArticleBySlug(decodedSlugForMySQL);
      if (mysqlArticle) {
        console.log(`✅ Artículo encontrado en MySQL: ${mysqlArticle.title}`);
        return mysqlArticle;
      }
      
      // Si no se encontró, intentar normalizando el slug (quitar colons, caracteres especiales)
      const normalizedMySQLSlug = decodedSlugForMySQL
        .replace(/:/g, '')       // quitar dos puntos
        .replace(/\s+/g, '-')    // espacios a guiones
        .replace(/-{2,}/g, '-')  // colapsar guiones múltiples
        .replace(/^-|-$/g, '');  // quitar guiones al inicio/final
      
      if (normalizedMySQLSlug !== decodedSlugForMySQL) {
        console.log(`Reintentando MySQL con slug normalizado: ${normalizedMySQLSlug}`);
        const mysqlArticle2 = await getMySQLArticleBySlug(normalizedMySQLSlug);
        if (mysqlArticle2) {
          console.log(`✅ Artículo encontrado en MySQL (normalizado): ${mysqlArticle2.title}`);
          return mysqlArticle2;
        }
      }
    } catch (dbErr) {
      console.error('Error al consultar artículo en MySQL:', dbErr);
    }
