...
export default function MobileMenu({
  ...
  const handleCategoryInteraction = (catId: string, catSlug: string) => {
    if (clickTimeoutRef.current) {
      ...
      const isMainCat = ['analisis-estrategico', 'desarrollo-web', 'posicionamiento'].includes(catSlug);
      router.push(isMainCat ? `/${catSlug}` : `/servicios/${catSlug}`);
      onClose();
    } else {
    ...
    >
      ...
      <div className="container mx-auto px-4 py-4 space-y-3 pb-8">
        ...
        <div className="space-y-2 mt-4">
          ...
          {/* Sección Software Personalizado con submenú */}
          <div className="border border-gray-100 rounded-lg overflow-hidden">
            <button
              onClick={() => setExpandedMenus(prev => ({ ...prev, software: !prev.software }))}
              ...
              aria-expanded={expandedMenus['software']}
            >
              <span>Software Personalizado</span>
              <ChevronDown
                className={`h-4 w-4 transition-transform duration-200 ${expandedMenus['software'] ? 'rotate-180' : ''}`}
                aria-hidden="true"
              />
            </button>

            <div
              className={`bg-gray-50 border-t border-gray-100 transition-all duration-200 overflow-hidden ${expandedMenus['software'] ? 'max-h-[800px]' : 'max-h-0'}`}
            >
              <Link
                href="/hoteles-y-restaurantes"
                className="block py-4 px-8 text-blue-600 font-bold border-b border-gray-100 bg-blue-50 transition-colors touch-manipulation"
                onClick={onClose}
              >
              ...
              <Link
                ...
                className="block py-4 px-8 text-gray-700 active:bg-gray-50 border-b border-gray-100 transition-colors touch-manipulation"
                onClick={onClose}
              >
                Motor de Reservas (Hoteles)
              </Link>
              <Link
                href="/menu-digital"
                className="block py-4 px-8 text-gray-700 active:bg-gray-50 border-b border-gray-100 transition-colors touch-manipulation"
              ...
              <Link
                ...
                className="block py-4 px-8 text-gray-700 active:bg-gray-50 transition-colors touch-manipulation"
                onClick={onClose}
              >
