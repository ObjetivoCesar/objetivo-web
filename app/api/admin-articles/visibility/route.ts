...
export async function PUT(request: Request) {
  try {
    ...
    try {
      ...
      await mysqlConn.execute('UPDATE articles SET published = ? WHERE slug = ?', [isVisible ? 1 : 0, slug]);
      await mysqlConn.end();
      console.log('MySQL Visibility Sync Successful for:', slug, 'isVisible:', isVisible);
    } catch (mysqlErr) {
      console.error('MySQL Visibility Sync Error:', mysqlErr);
    }
